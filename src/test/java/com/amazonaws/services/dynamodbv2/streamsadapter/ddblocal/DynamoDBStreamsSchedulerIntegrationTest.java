 ...
 */
public class DynamoDBStreamsSchedulerIntegrationTest extends DynamoDBStreamsLocalTestBase {

    @Test
    public void testSchedulerLifecycle() throws Exception {
        String tableName = "SchedulerIntegrationTestTable";
        String streamArn = createTableWithStream(tableName);
        String appName = "SchedulerIntegrationTest";
        ...
        putItem(tableName, "s1", "schedulerVal1");
        putItem(tableName, "s2", "schedulerVal2");

        new Thread(scheduler, "kcl-scheduler").start();

        try {
            await().atMost(Duration.ofSeconds(180))
                    .untilAsserted(() -> assertTrue(recordCount.get() >= 2));
        } finally {
